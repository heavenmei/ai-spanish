...
Page({
  ...
  async getSearchResult(keyword, skip = 0, getLemma = true, clear = true) {
    ...
    const expUnallow = new RegExp("[^A-Za-z- \u4e00-\u9fa5'.]", "g");
    const expSymbol = new RegExp("^[ -'.]+$", "g");
    // let zhExp = /[\u4e00-\u9fa5]/
    const isInvalid = expUnallow.test(keyword);
    const onlySymbol = expSymbol.test(keyword);
    ...
    const res = await WordApi.getSearchResult({
      keyword,
      DBtype,
      skip,
      getLemma,
  ...
  transResult(searchresult, keyword, clear = true) {
    ...
    for (let i = 0; i < lemmares.length; i++) {
      ...
      if (lemmares[i].translation.indexOf("\n") != -1) {
        lemmares[i].translation = lemmares[i].translation.substring(
          0,
          lemmares[i].translation.indexOf("\n")
        );
      }
    }
    ...
    for (let i = 0; i < directres.length; i++) {
      if (directres[i].translation.indexOf("\n") != -1) {
        directres[i].translation = directres[i].translation.substring(
          0,
  ...
  getWordDetail(e) {
    ...
    const type = parseInt(e.currentTarget.dataset.sourcetype); // dataset不区分大小写
    // console.log('index:', index, 'type:', type)
    const type_l = ["lemmaResult", "directResult", "history"];
    const wordObjList = this.data[type_l[type]];
    const wordObj = wordObjList[index];
