...
const toExchangeList = (exchange) => {
  ...
  const strList = exchange.split("/");
  const exchangeDict = {};
  const exchangeList = [];
  let lemma = "";
  let type_1_exchange = [];
  for (let i = 0; i < strList.length; i++) {
    const exchangeType = strList[i].split(":");
    if (exchangeType[0] == "0") {
...
const toTransList = (translation) => {
  ...
  const l = translation.split("\n");
  const transList = [];
  for (let i = 0; i < l.length; i++) {
    const spaceIndex = l[i].indexOf(" "); // 找到第一个空格，空格前为词性，空格后为释义
    let pos = "";
    let meaning = l[i];
    if (spaceIndex != -1) {
...
const getTagList = (wordDetail) => {
  const originTagList = wordDetail.tagList;
  const tagList = [];
...
const getWordVoiceUrl = (word, source = 0, type = 2) => {
  ...
  if (globalData.isLogin && globalData.userInfo.settings.type)
    type = globalData.userInfo.settings.type;
  let url = "";
  if (source == 0) {
...
const handleWordDetail = (wordDetail, settings = {}) => {
  ...
  if (wordDetail.definition)
    wordDetail.definition = toTransList(wordDetail.definition);
  if (wordDetail.exchange)
    wordDetail.exchange = toExchangeList(wordDetail.exchange);
  if ("getShortTrans" in settings && settings.getShortTrans) {
    const transList = JSON.parse(JSON.stringify(wordDetail.translation));
    const shortTransList = transList.slice(
      0,
      transList.length > 5 ? 5 : transList.length
    );
    for (let i = 0; i < shortTransList.length; i++) {
      const str = shortTransList[i].meaning;
      if (str.length > 20) {
        const cutIndex = str.lastIndexOf(",", 18);
        if (cutIndex != -1)
          shortTransList[i].meaning = `${str.substring(0, cutIndex)} ...`;
      }
    }
  ...
  if (wordDetail.sample_list) {
    ...
    for (let i = 0; i < wordDetail.sample_list.length; i++) {
      ...
      if (i != wordDetail.sample_list.length - 1)
        transItem = toTransList(wordDetail.sample_list[i].translation)[0];
      const meaningStr = transItem.meaning;
      if (meaningStr.length > 23) {
        const cutIndex = meaningStr.lastIndexOf(",", 24);
        if (cutIndex != -1)
          transItem.meaning = meaningStr.substring(0, cutIndex);
      }
